class XpCheckServer < SOAP::RPC::StandaloneServer
	def credit_check (user, cost, item, quantity)	
		...
		begin
			...
			# after XpLevel is found, the 2nd and 3rd following bytes represent the level
	...
	def take_exp (user, cost, quantity)	
		...
		return `su minecraft bash -c "screen -p 0 -S TekkitServer -X eval 'stuff \"$(eval echo "xp -#{amount}L #{user}")\"\015'"`
	end
end